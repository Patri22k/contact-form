...
const SuccessfulMessage = ({ show }) => {
    return (
        <div
            className={`successful-message fixed left-1/2 transform -translate-x-1/2 bg-[#2B4246] text-white rounded-xl transition-transform duration-500 ease-out ${show ? "translate-y-0 opacity-100 py-2 pl-6 top-1 text-xs sm:text-sm sm:top-3 md:text-md md:top-3 lg:text-base lg:top-5 lg:w-1/4 lg:py-6 2xl:" : "-translate-y-full opacity-0 pointer-events-none"}`}
        >
